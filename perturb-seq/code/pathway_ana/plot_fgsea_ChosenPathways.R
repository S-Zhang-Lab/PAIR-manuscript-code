...
chosen_clusters <- c(5, 7, 12) # DNA repair related pathway clusters

pathway_embed <- pathway_embed[pathway_embed$seurat_clusters %in% chosen_clusters, ]

# pathway_embed$seurat_cluster <- as.factor(pathway_embed$seurat_cluster)
# ggsave(
#   "./res/2025_1217/pathway_umap.png",
#   width = 6,
#   height = 5,
#   dpi = 300
# )
# ggplot(pathway_embed, aes(umap_1, umap_2, color = seurat_cluster)) +
...
input_dir  <- "./res/2025_1203/pwy_results_RNPoverCTRL"

output_dir_overall <- "./res/2025_1217/pwy_results_RNPoverCTRL/overall"
output_dir_NES_larger1 <- "./res/2025_1217/pwy_results_RNPoverCTRL/NES_larger1"
output_dir_sig <- "./res/2025_1217/pwy_results_RNPoverCTRL/sig"

# Create output folder if it does not exist
if (!dir.exists(output_dir_overall)) {
  dir.create(output_dir_overall, recursive = TRUE)
}

if (!dir.exists(output_dir_NES_larger1)) {
  dir.create(output_dir_NES_larger1, recursive = TRUE)
}

if (!dir.exists(output_dir_sig)) {
  dir.create(output_dir_sig, recursive = TRUE)
}
